...
카테고리 분포 바, 필터 칩 개수, 최근 30일 그래프가 **전부 자동으로 갱신됩니다.**

`duration` 을 적지 않으면 그 세션은 Mat Time 합계에 0분으로 잡히고 30일 그래프에서
...
`/workout/` 아래쪽 Training Balance 안에, 깃허브 잔디처럼 하루 한 칸씩 최근 30일을
보여주는 그래프가 있습니다. 칸 색은 그날 훈련 시간으로 정해집니다.
...
칸에 마우스를 올리면 날짜·세션 수·훈련 시간이 뜨고, 그날 기록이 하나면 눌러서 바로
그 일지로 갈 수 있습니다.

이 그래프만 브라우저에서 그립니다. Liquid 로 미리 그리면 **마지막 빌드 시점**에 30일
창이 고정되어, 한동안 푸시가 없으면 "최근 30일"이 실제로는 몇 주 전 구간을 가리키게
됩니다. 보는 시점의 오늘을 기준으로 맞추기 위해 런타임에 계산합니다.

기간을 바꾸려면 `workout/index.html` 의 `data-days="30"` 값만 고치면 됩니다.
